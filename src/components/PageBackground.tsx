...
/** Same vial SVG used in LowTicketBackground — dark stopper, clear glass body */
function VialSvg() {
  return (
    <svg
      viewBox="0 0 54 130"
    ...
    >
      ...
      <rect x="7" y="2" width="40" height="28" rx="4" fill="#1c1c1c" />
      <rect x="7" y="2" width="40" height="9" rx="4" fill="rgba(255,255,255,0.10)" />
      <rect x="7" y="24" width="40" height="3" fill="rgba(255,255,255,0.07)" />
      ...
      <rect x="3" y="28" width="48" height="98" rx="5"
        fill="none" stroke="rgba(255,255,255,0.60)" strokeWidth="1.2" />
      <rect x="9" y="33" width="7" height="86" rx="4" fill="rgba(255,255,255,0.09)" />
      <rect x="40" y="33" width="5" height="86" rx="3" fill="rgba(0,0,0,0.14)" />
...
export default function PageBackground({ showVials = true }: Props) {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (vialsRef.current) {
        vialsRef.current.style.transform = `translate(${-x * 0.7}px, ${-y * 0.7}px)`;
      }
    };
    ...
    <>
      ...
      {showVials && (
        <div ref={vialsRef} className={s.vials}>
          ...
          <div className={s.vial4}><VialSvg /></div>
        </div>
      )}
    </>
  );
